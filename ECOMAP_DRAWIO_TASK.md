# タスク: draw.io エコマップ生成機能の統合

> **Claude Code への指示**: このファイルを読み込んだら、記載された手順に従って実装を進めてください。
> 既存コードベースの構造・規約を尊重し、`lib/db_operations.py` の `run_query()` を再利用してください。

---

## 1. 背景と目的

このプロジェクト（親亡き後支援データベース）に、**draw.io形式のエコマップ（支援ネットワーク図）生成機能**を統合します。

エコマップは、障害のあるクライアントを中心に、禁忌事項・推奨ケア・キーパーソン・後見人・医療機関・手帳情報・特性・支援者・利用サービスを放射状に配置した関係図です。

### なぜ draw.io か

- SVG/PNG と違い、ユーザーが**後から編集**できる（ノード追加・位置変更・コメント追記）
- draw.io（diagrams.net）は**無料アプリ**でデスクトップ版もWeb版もある
- `.drawio` ファイルの実体はXMLなので、Pythonで完全に生成可能

### 成果物

1. `skills/ecomap_generator/drawio_engine.py` — draw.io XML生成エンジン
2. `pages/ecomap.py` — Streamlit UIページ
3. `app.py` のナビゲーション更新
...
```
neo4j-agno-agent/
├── app.py                          ← ナビゲーションに「エコマップ」追加
├── skills/
│   ├── report_generator/           ← 既存（PDF/Excel出力）
│   └── ecomap_generator/           ← ★新規作成
│       ├── __init__.py
│       └── drawio_engine.py        ← draw.io XML生成エンジン
└── pages/
    ├── home.py                     ← 既存
    ├── client_list.py              ← 既存
    ├── claude_guide.py             ← 既存
    └── ecomap.py                   ← ★新規作成（Streamlit UI）
```
...
### Step 1: `skills/ecomap_generator/__init__.py` を作成

空ファイルまたは最小限のインポート。

### Step 2: `skills/ecomap_generator/drawio_engine.py` を作成

このファイルがコア。以下のセクションにソースコード全文を掲載しています。

**重要な設計変更点**（元のプロトタイプからの移植時に注意）：

- **Neo4j接続は `lib.db_operations.run_query()` を使う**（独自ドライバーは作らない）
- `lib.db_operations.is_db_available()` でDB接続チェック
- パッケージ構造: `skills.ecomap_generator.drawio_engine` としてインポート可能に

### Step 3: `pages/ecomap.py` を作成

Streamlit UIページ。クライアント選択 → テンプレート選択 → 生成 → ダウンロードの流れ。

### Step 4: `app.py` のナビゲーション更新

「活用」セクションにエコマップページを追加。
...
`CLAUDE.md` と `claude-skills/ecomap-generator/SKILL.md` に定義済み。
**正式なリレーション名**（`docs/NEO4J_SCHEMA_CONVENTION.md` 参照）：

```cypher
-- 禁忌事項（正式名: MUST_AVOID）
(:Client)-[:MUST_AVOID]->(:NgAction)
-- ※旧名 PROHIBITED が残存している場合あり。読み取りは [:MUST_AVOID|PROHIBITED] で対応

-- 推奨ケア（正式名: REQUIRES）
(:Client)-[:REQUIRES]->(:CarePreference)
-- ※旧名 PREFERS が残存している場合あり。読み取りは [:REQUIRES|PREFERS] で対応

-- キーパーソン（正式名: HAS_KEY_PERSON）
(:Client)-[:HAS_KEY_PERSON {rank: int}]->(:KeyPerson)
-- ※旧名 EMERGENCY_CONTACT が残存している場合あり。読み取りは [:HAS_KEY_PERSON|EMERGENCY_CONTACT] で対応

-- 後見人（正式名: HAS_LEGAL_REP）
(:Client)-[:HAS_LEGAL_REP]->(:Guardian)
-- ※旧名 HAS_GUARDIAN が残存している場合あり。読み取りは [:HAS_LEGAL_REP|HAS_GUARDIAN] で対応

-- 医療機関
(:Client)-[:TREATED_AT]->(:Hospital)

-- 手帳・証明書（正式名: HAS_CERTIFICATE）
(:Client)-[:HAS_CERTIFICATE]->(:Certificate)
-- ※旧名 HOLDS が残存している場合あり。読み取りは [:HAS_CERTIFICATE|HOLDS] で対応
...
-- 利用サービス
(:Client)-[:USES_SERVICE {status: str}]->(:ServiceProvider)
```

### NgAction の riskLevel 値
- `LifeThreatening` — 生命に関わる（最優先）
- `Panic` — パニック誘発
- `Discomfort` — 不快・苦痛

### CarePreference の priority 値
- `High`, `Medium`, `Low`

### KeyPerson のランク
- `rank` または `priority` プロパティ（数値、1が最優先）

---

## 5. drawio_engine.py の完全ソースコード

以下を `skills/ecomap_generator/drawio_engine.py` として作成してください。
**`lib.db_operations` を使うよう書き換え済み**です。
...
## 6. pages/ecomap.py のStreamlit UI仕様

### UI要件

1. **クライアント選択**: `lib.db_operations.get_clients_list()` を使った selectbox
2. **テンプレート選択**: 4種のテンプレートから radio で選択（説明つき）
3. **プレビュー情報**: 選択中クライアントの登録データ数をカテゴリ別に表示
4. **生成ボタン**: 「エコマップ生成」ボタン押下で XML 生成
5. **ダウンロード**: `st.download_button` で `.drawio` ファイルをダウンロード
6. **draw.io案内**: 「draw.ioで開く方法」を expander で説明

### UIデザイン参考

既存の `pages/client_list.py` のカードデザイン、`app_narrative.py` のサイドバー構成を参考にすること。
CSSスタイルは既存ページと統一感を持たせる。
...
- DB未接続時: `st.warning()` で警告しつつ、サンプルデータでのデモ生成を許可
- テンプレート選択: `st.radio` + 各テンプレートの `use_case` を `st.caption` で表示
- ダウンロードファイル名: `{クライアント名}_ecomap_{テンプレート名}_{日付}.drawio`
- draw.io案内:
  - Web版: https://app.diagrams.net/ にアクセスしてファイルを開く
  - デスクトップ版: https://github.com/jgraph/drawio-desktop/releases からインストール

---

## 7. app.py の変更

ナビゲーションに「エコマップ」を追加する。「活用」セクションまたは新しい「可視化」セクションに配置。
...
# サンプルデータでテスト（DB不要）
uv run python -c "
from skills.ecomap_generator.drawio_engine import generate_drawio_xml
xml = generate_drawio_xml('テスト太郎', template='full_view')
with open('/tmp/test_ecomap.drawio', 'w') as f:
    f.write(xml)
print('✅ 生成成功:', len(xml), 'bytes')
"
```
...
# ブラウザで http://localhost:8501 → 「エコマップ」ページを確認
```

### 8.3 確認項目

- [ ] `skills/ecomap_generator/drawio_engine.py` がインポートエラーなく動作
- [ ] DB未接続時にサンプルデータでフォールバックする
- [ ] DB接続時に実データからエコマップが生成される
- [ ] 4種すべてのテンプレートが正常に動作する
- [ ] `.drawio` ファイルが draw.io アプリで正常に開ける
- [ ] Streamlit UIでダウンロードボタンが動作する
- [ ] app.py のナビゲーションに「エコマップ」が表示される

---

## 9. 追加の依存パッケージ

なし。draw.io XML はPython標準ライブラリ（`math`, `xml`, `dataclasses`）のみで生成可能。
`pyproject.toml` の変更は不要。

---

## 10. CLAUDE.md の更新

完了後、`CLAUDE.md` に以下を追記すること：

### File Organization セクション

```
├── skills/
│   ├── report_generator/           # PDF/Excel出力
│   └── ecomap_generator/           # ★ draw.io エコマップ生成
│       ├── __init__.py
│       └── drawio_engine.py
├── pages/
│   └── ecomap.py                   # ★ エコマップ Streamlit UI
```

### System Components セクション
