...
Windows ではデスクトップにショートカットを作成して、ダブルクリックだけで全サービスを起動できます。

#### ショートカットの作成手順

1. PowerShell を開きます（スタートメニューで「PowerShell」を検索）
2. プロジェクトフォルダに移動します:
```powershell
cd $HOME\Documents\neo4j-agno-agent
```
3. ショートカット作成スクリプトを実行します:
```powershell
powershell -ExecutionPolicy Bypass -File scripts\create-desktop-shortcut.ps1
```

これでデスクトップに2つのショートカットが作成されます:

| ショートカット | アイコン | 機能 |
|---------------|---------|------|
| **親なき後支援DB 起動** | 紫の盾+ハート | Neo4j・API・フロントエンドを一括起動し、ブラウザを自動で開く |
| **親なき後支援DB 停止** | 赤い停止ボタン | 全サービスを一括停止 |

#### 起動の流れ

1. **Docker Desktop** が起動していることを確認（タスクトレイのクジラアイコン）
2. デスクトップの **「親なき後支援DB 起動」** をダブルクリック
3. 黒いウィンドウ（コマンドプロンプト）が開き、以下が自動で実行されます:
   - 依存パッケージの確認・インストール
   - Claude Desktop Skills のインストール
   - Neo4j データベースの起動
   - API サーバーの起動（バックグラウンド）
   - フロントエンドの起動（バックグラウンド）
   - ブラウザで `http://localhost:3001` を自動で開く
4. 「起動完了」メッセージが表示されたら準備完了です

#### 停止の流れ

デスクトップの **「親なき後支援DB 停止」** をダブルクリックすると、全サービスがまとめて停止します。

> **初回のみ**: `.env` ファイルがない場合、自動でメモ帳が開きます。APIキーを入力して保存してください。

#### 手動で start.bat を使う場合

ショートカットを作成しなくても、プロジェクトフォルダ内の `start.bat` をダブルクリックすれば同じように起動できます。
