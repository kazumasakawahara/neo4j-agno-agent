...
## Feature: Audio Meeting Records & Client Similarity - 2026-03-12

### Summary
2つの応用機能を追加。(A) 面談音声をネイティブembeddingし、テキストクエリで検索可能にする。(B) クライアント支援概要のembeddingによる類似度分析。

### 機能A: 音声面談記録
- **embed_audio()**: Gemini Embedding 2 で音声を直接embedding（最大80秒）
- **transcribe_audio()**: Gemini 2.0 Flash による自動文字起こし
- **register_meeting_record()**: 音声→embedding+文字起こし+Neo4j登録の一括パイプライン
- **search_meeting_records_semantic()**: テキスト/音声ネイティブ両方のインデックスで検索可能
- **登録UI**: `pages/meeting_record.py` — 音声アップロード、プレビュー、自動文字起こし
- **MeetingRecord ノード**: date, title, duration, filePath, mimeType, transcript, note, embedding[768], textEmbedding[768]
- **RECORDED リレーション**: Supporter → MeetingRecord → (ABOUT) → Client

### 機能B: クライアント類似度分析
- **build_client_summary_text()**: Client関連ノード（Condition, NgAction, CarePreference, SupportLog）を集約して概要テキスト構築
- **embed_client_summary()**: task_type="CLUSTERING" で summaryEmbedding 生成・付与
- **find_similar_clients()**: ベクトルインデックスで類似クライアント検索
- **search_similar_clients_by_text()**: テキスト説明からの類似検索（新規利用者の特徴入力用）
- **自動付与フック**: `register_to_database()` でClient関連ノード登録時にsummaryEmbeddingを自動再計算
- **バックフィル**: `uv run python scripts/backfill_embeddings.py --label Client`

### 設計判断
- 音声80秒超はembeddingスキップ、文字起こし＋テキストembeddingのみ（ベストエフォート）
- `ffprobe` が未インストールの場合は長さチェックをスキップしAPI側のエラーに任せる
- Client の summaryEmbedding は `task_type="CLUSTERING"` を使用（類似度比較に最適化）
- MeetingRecord は embedding（音声ネイティブ）と textEmbedding（transcript/note）の2つのベクトルを持つ
